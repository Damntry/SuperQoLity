...
namespace SuperQoLity.SuperMarket.PatchClassHelpers.Employees.RestockMatch.Component {
	...
	public class RestockMatcher : NetworkBehaviour {
		...
		public void Awake() {
			...
			AncientRestockLogic.queueRestockJobsOld = new();
			AncientRestockLogic.IsRestockGenerationWorking = false;
			lastTimeCheck = -1;
			currentRestockProcessInterval = RestockJobsManager.RestockProcessNotPossibleInterval;
		}

		public void FixedUpdate() {
			if (AncientRestockLogic.IsRestockGenerationWorking) {
				return;
			}
		...
		private bool IsRestockPossible(NPC_Manager __instance) {
			return __instance.storageOBJ.transform.childCount > 0 &&
				...
				EmployeeJobAIPatch.HasRestockingEmployeAssigned(__instance);
		}

		private static (int totalCount, float maxEmployeeSpeed) GetRestockEmployeeStats(NPC_Manager __instance) {
			int totalCount = 0;
			float maxEmployeeSpeed = -1;

			__instance.employeeParentOBJ.transform
				.Cast<Transform>()
				.Do(
					(t) => { 
						float employeeSpeed = t.GetComponent<NavMeshAgent>().speed;
						maxEmployeeSpeed = employeeSpeed > maxEmployeeSpeed ? employeeSpeed : maxEmployeeSpeed;

						totalCount += t.GetComponent<NPC_Info>().taskPriority == 2 ? 1 : 0;
					}
				);

			return (totalCount, maxEmployeeSpeed);
		}
		...
		private int CalculateMaxQueuedJobsForCycle(NPC_Manager __instance) {
			int restockEmployeeCount = EmployeeJobAIPatch.GetEmployeeCount(__instance, 2);
		...
		private static Task RestockJobGeneration(int maxJobsRestockCycle,
			...
			RestockJob<ProductShelfInfo> possibleRestockJobs = new(jobsPerPriority);
		...
		private static Dictionary<int, List<ShelfSlotData>> GenerateStorageDictionary(NPC_Manager __instance) {
			...
			ContainerSearchLambdas.ForEachStorageSlotLambda(__instance, checkNPCStorageTarget: false, skipEmptyBoxes: true,
				(storageIndex, slotIndex, productId, quantity, storageObjT) => {
					...
					List<ShelfSlotData> list;
					if (!dictStorage.TryGetValue(productId, out list)) {
						dictStorage.Add(productId, list = new());
					}
					list.Add(new ShelfSlotData(storageIndex, slotIndex, productId, quantity, 
