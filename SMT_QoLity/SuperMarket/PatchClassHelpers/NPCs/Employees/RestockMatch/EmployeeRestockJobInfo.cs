...
namespace SuperQoLity.SuperMarket.PatchClassHelpers.NPCs.Employees.RestockMatch {
	...
	public static class EmployeeRestockJobInfo {
		...
		public static void SetRestockJobInfo(this NPC_Info npcInfo, RestockJobInfo jobInfo) {
			...
			npcInfo.SetProductAvailableArray(jobInfo);
		}
		...
		public static bool UpdateRestockJobInfo(this NPC_Info npcInfo, ProductShelfSlotInfo shelfSlotInfo, int maxProductsPerRow) {
			if (shelfSlotInfo == null) {
				TimeLogger.Logger.LogFatal($"The parameter shelfSlotInfo is null for npc {npcInfo.netId}", 
					LogCategories.AI);
				return false;
			}
			...
			if (!exist) {
				TimeLogger.Logger.LogFatal($"An existing ProductAvailableInfo couldnt be found for npc {npcInfo.netId}",
					LogCategories.AI);
				return false;
			}
			...
			npcInfo.SetProductAvailableArray(jobInfoNew);
		...
		private static void SetProductAvailableArray(this NPC_Info npcInfo, RestockJobInfo jobInfo) {
            ...
            //Even though I ve replaced every existing use of productAvailableArray (except for manufacturing), set
            //	its values in case another mod or the base code needs them somewhere else in the future.
