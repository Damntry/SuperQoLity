...
namespace SuperQoLity.SuperMarket.PatchClassHelpers.NPCs.JobScheduler.Helpers {
	...
	public class FrequencyTrendCalculation {
		...
		public float CalculateJobFreqStepValue(float averageWaitTimeMillis, float lastAvgWaitTime,
			float lastJobFreqMult, float fixedDeltaTime, AutoModeData autoModeData, NPCType npcType) {
			...
			//	per second, this means that if all npcs were to not skip work: 1000 / 50 = 20ms.
            ...
            if (autoModeData.AvgWaitTargetFromNpcType(npcType) >= 0) {
                //Take predefined target for this auto mode and npc type.
				...
				//	enough that all npcs are processed in a single loop more than once.
				minimumBaseline = 0;
			}
