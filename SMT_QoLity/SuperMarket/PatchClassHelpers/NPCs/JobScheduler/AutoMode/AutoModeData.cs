...
namespace SuperQoLity.SuperMarket.PatchClassHelpers.NPCs.JobScheduler.AutoMode {
	...
	public abstract class AutoModeData {
		...
		/// <summary>
		/// Each second, the job scheduler measures how much an npc had to wait to be 
		/// able to start its own job, and then calculates an average between all npcs. 
		/// The value of AvgNpcWaitTargetMillis is the target for the job scheduler to
        ...
        public float AvgCustomerWaitTargetMillis { get; init; }
        ...
        /// </param>
		/// <param name="avgCustomerWaitTargetMillis">Avg target wait for customers.</param>
        ...
        protected AutoModeData(EnumJobFrequencyMultMode jobFreqMode, float defaultFrequencyMult, float avgEmployeeWaitTargetMillis,
                float avgCustomerWaitTargetMillis, float minFreqMult, float maxFreqMult, float decreaseStep, float increaseStep) {

			JobFreqMode = jobFreqMode;

			DefaultFrequencyMult = defaultFrequencyMult;
			AvgEmployeeWaitTargetMillis = avgEmployeeWaitTargetMillis;
            AvgCustomerWaitTargetMillis = avgCustomerWaitTargetMillis;

            MinFreqMult = minFreqMult;
			MaxFreqMult = maxFreqMult;
		...
		public float AvgWaitTargetFromNpcType(NPCType npcType) =>
			npcType switch {
				NPCType.Employee => AvgEmployeeWaitTargetMillis,
                NPCType.Customer => AvgCustomerWaitTargetMillis,
				_ => throw new NotSupportedException(npcType.ToString())
            };
        ...
        private int VerifyLocalValues() {
			...
			(float value, AutoModeValueLimit limits)[] AllValueLimits = [
				...
				(AvgEmployeeWaitTargetMillis,		AutoModeLimits.AvgNpcWaitTarget),
                (AvgCustomerWaitTargetMillis,       AutoModeLimits.AvgNpcWaitTarget),
		...
		private void NotifyErrors(int notificationCounter) {
			...
			if (IsCustomMode) {
				TimeLogger.Logger.LogWarningShowInGame($"{notificationCounter} value/s of Custom auto mode were " +
				$"outside allowed limits", LogCategories.JobSched);
			} else {
				//I fucked up.
				throw new InvalidOperationException($"{notificationCounter} value/s of the " +
					$"{ModConfig.Instance.NpcJobFrequencyMode.Value} mode were out of " +
		...
		public float Clamp(float jobFreqMultiplier) {
			if (float.IsNaN(jobFreqMultiplier)) {
				TimeLogger.Logger.LogWarning("Calculated Job Frency Multiplier is NaN. " +
