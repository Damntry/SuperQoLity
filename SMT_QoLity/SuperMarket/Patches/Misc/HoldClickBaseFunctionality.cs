...
namespace SuperQoLity.SuperMarket.Patches.Misc {
    ...
    /*
	public class HoldClickBaseFunctionality : FullyAutoPatchedInstance {
		...
		public override string ErrorMessageOnAutoPatchFail { get; protected set; } = 
			$"{MyPluginInfo.PLUGIN_NAME} - Hold click to clear debris from pillars patch failed. Disabled.";


		private LayerMask interactableMask = -1;

		private FixedCapacityUniqueQueue<ProductCheckoutSpawn> DebrisQueue;
		...
		public override void OnPatchFinishedVirtual(bool IsPatchActive) {
			if (IsPatchActive) {
				DebrisQueue = new(MaxItemsQueue, true);
		...
		private void InitState() {
			...
			if (interactableMask < 0) {
				interactableMask = SMTInstances.PlayerNetwork().interactableMask;
			}
		}
		...
		public void ProcessCheckoutProductPickup(float currentTime, Player mainPlayerControl) {
			...
			if (mainPlayerControl.GetButton(scanProdAction)) {
				...
				if (Physics.Raycast(Camera.main.transform.position,
				...
				} else if (nextCheckoutProductPickTime < currentTime && DebrisQueue.Count > 0) {
					...
					//	long enough from last successful product raycast to stop dequeing.
					if (DequeueTimeLimit >= currentTime) {
						PerformCheckoutProductClick(DebrisQueue.Dequeue(), currentTime);
					} else {
						DebrisQueue.Clear();
					}
				}
		...
		private void CheckoutProductAimed(float currentTime, ProductCheckoutSpawn productBelt) {
			if (nextCheckoutProductPickTime < currentTime) {
				...
				if (DebrisQueue.TryEnqueue(productBelt, out _)) {
