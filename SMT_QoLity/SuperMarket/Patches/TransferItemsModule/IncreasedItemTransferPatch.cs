...
namespace SuperQoLity.SuperMarket.Patches.TransferItemsModule {
    ...
    public class IncreasedItemTransferPatch : FullyAutoPatchedInstance {
        ...
        public override void OnPatchFinishedVirtual(bool IsPatchActive) {
            if (IsPatchActive) {
                NetworkSpawnManager.RegisterNetwork<ItemTransferNetwork>(ItemTransferNetwork.NetworkAssetId);
            }
        }
...
#pragma warning restore Harmony003 // Harmony non-ref patch parameters modified
        ...
        public static bool IsItemTransferEnabledFor(CharacterType charType, int numMovedProducts) {
            ...
            //If not disabled, its always active while the store is closed.
        ...
        //TODO 5 - Change all of these methods to fully use CodeMatcher instead.
        ...
        private static IEnumerable<CodeInstruction> AddItemToRowTranspiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator) {
            ...
            FieldInfo extraParam2Field = AccessTools.Field(typeof(PlayerNetwork), nameof(PlayerNetwork.extraParameter2));

            Label? exitBranchLabel = null;
            int branchStartPos =
                codeMatcher
                    .End()
                    .MatchEndBackwards(			//Find last occurrence of extraParam2Field
						new CodeMatch(OpCodes.Ldfld, extraParam2Field)
                    ).MatchEndBackwards(        //Search back to start of branch
						new CodeMatch(c => c.Branches(out exitBranchLabel)
                    )
                ).Pos + 1;
            ...
            int branchEndPos = codeMatcher
                .MatchForward(true,
                    new CodeMatch(inst => inst.labels.Contains(exitBranchLabel.Value))
                ).Pos;

            var instructionList = codeMatcher.InstructionEnumeration().ToList();

            return ReplaceConstantsWithCallResult(instructionList, generator, branchStartPos, branchEndPos, RowActionType.Add);
        }
        ...
        private static IEnumerable<CodeInstruction> TranspilerChangeProductTransferCount(IEnumerable<CodeInstruction> instructions, ILGenerator generator, RowActionType rowActionType) {
            ...
            return ReplaceConstantsWithCallResult(instrList, generator, indexStart, indexEnd, rowActionType);
...
#pragma warning disable Harmony003 // Harmony non-ref patch parameters modified
        private static IEnumerable<CodeInstruction> ReplaceConstantsWithCallResult(List<CodeInstruction> instrList, ILGenerator generator, int indexStart, int indexEnd, RowActionType rowActionType) {
            ...
            if (rowActionType == RowActionType.Add) {
                ...
                //TODO 3 - This entire thing done this way is too unsafe.
                //Move the achievement related instructions to the end of the else.
                int numAchievInstructions = 4;
                instrList.InsertRange(indexEnd, instrList.GetRange(indexStart, numAchievInstructions));
                instrList.RemoveRange(indexStart, numAchievInstructions);
                indexEnd -= numAchievInstructions;	//We dont want to operate in the range of the achievement instructions anymore, so count them out.
            }
            ...
            indexEnd += callGetNumTransferItemsInstr.Count + 1;

            CodeMatcher codeMatcher = new (instrList);

            //Replace the constants placing a value of 1 in the stack, with the value of our local var
            int numMatches = 0;
            codeMatcher.Advance(indexStart);

            while (codeMatcher.MatchForward(false, new CodeMatch(inst => inst.LoadsConstant(1))).IsValid &&
                    codeMatcher.Pos <= indexEnd) {

                codeMatcher.SetInstructionAndAdvance(loadLocalVarNumItemsTransfer);
                numMatches++;
            }

            if (numMatches < 2) {
                //In reality its 3 matches for RowActionType.Add but better keep it lower.
                throw new TranspilerDefaultMsgException($"Only {numMatches} matches where found of the expected " +
                    $"2.");
            }
...
#pragma warning restore Harmony003 // Harmony non-ref patch parameters modified
        ...
        private static List<CodeInstruction> CreateILCall_GetNumTransferItems(List<CodeInstruction> instrList, int indexStart, int indexEnd, RowActionType rowActionType) {
            List<CodeInstruction> GetNumTransferItemsInstr = new ();

            //List<CodeInstruction> getBoxCountILInstr = GetBoxCountILInstructions(instrList, indexStart, indexEnd);
            CodeInstruction getBoxCountILInstr = GetBoxCountILInstruction();
            ...
            GetNumTransferItemsInstr.Add(Transpilers.EmitDelegate<Func<int, int, int, CharacterType, int>>(GetNumTransferItems));
        ...
        private static List<CodeInstruction> GetILParametersAddingItems(List<CodeInstruction> instrList, CodeInstruction getBoxCountILInstr) {
            ...
            paramsILinstr.Add(getBoxCountILInstr);     //Duplicate the IL lines that put the value of "component.extraParameter2" on the stack as 1º parameter
        ...
        private static List<CodeInstruction> GetILParametersRemovingItems(List<CodeInstruction> instrList, CodeInstruction getBoxCountILInstr) {
            ...
            FieldInfo maxItemsPerBoxField = AccessTools.Field(typeof(ProductData), nameof(ProductData.maxItemsPerBox));
            ...
            paramsILinstr.Add(getBoxCountILInstr);     ////Duplicate the IL lines that put the value of "component.extraParameter2" on the stack as 2º parameter
        ...
        private static CodeInstruction GetBoxCountILInstruction() =>
            Transpilers.EmitDelegate(
                () => FirstPersonController.Instance.GetComponent<PlayerNetwork>().extraParameter2
            );
        ...
        private static List<CodeInstruction> GetBoxCountILInstructions(List<CodeInstruction> instrList, int indexStart, int indexEnd) {
            ...
            return [instrList[indexDup - 1], instrList[indexDup + 1]];
        }
		*/
